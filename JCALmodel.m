function Absorption = JCALmodel(tort,sigma,epsilon,viscous,thermal,k0,thick,freq)
...
K_eq=(gamma*P0/epsilon)./(gamma-(gamma-1)./(1-i*((epsilon*eta)./(k0*Npr.*omega*rho0)).*sqrt(1+i*(4*k0^2*rho0.*omega*Npr)/(eta*epsilon^2*thermal^2))));
Zc=(K_eq.*rho_eq).^0.5;
%Complex wave number
